...
static void BM_Run_Single_Threaded_Simulation( benchmark::State& state ) {
  ...
  for( auto _ : state ) {
    Run_Single_Threaded_Simulation( total_runs, total_timesteps, seed, do_write_to_file, parameters );
  }
}

BENCHMARK( BM_Run_Single_Threaded_Simulation );
...
static void BM_Run_Multi_Threaded_Simulation( benchmark::State& state ) {
  ...
  for( auto _ : state ) {
    Run_Multi_Threaded_Simulation( total_runs, total_timesteps, seed, do_write_to_file, parameters );
  }
}

BENCHMARK( BM_Run_Multi_Threaded_Simulation );
